# app/dashboard.py (Versione Corretta)
...
if st.button("Esegui Analisi e Genera Segnale"):
    ...
    if market_data is not None and cmi_data is not None:
        ...
        # Controlliamo se il DataFrame ha dati PRIMA di accedere all'ultima riga
        if df_signals.empty:
            st.warning("Calcolo completato, ma nessun segnale generato per l'intervallo di date corrente. Potrebbe essere dovuto a dati mancanti o ritardi nelle fonti.")
        else:
